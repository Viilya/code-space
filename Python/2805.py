...
treeSum = 0
longestTree = max(trees)
middle = max(trees) // 2
#print(middle)
tmp = 0
for s in range(middle * 2):
    treeSum = 0;
    tmp = middle
    #print("cut Tree : ", end = "")
    for i in range(n):
    ...
    if(treeSum > m):
        if(preMiddle > middle):
            middle = (preMiddle + middle) // 2
        else:
            middle = middle * 3 // 2 - preMiddle // 2
        #print("Too much ",tmp, treeSum)
    elif(treeSum < m):
        if(preMiddle < middle):
            middle = preMiddle // 2 + middle // 2
        else:
            middle = middle * 3 // 2 - preMiddle // 2
        #print("Too smal ",tmp, treeSum)
    else:
        #print("just Fit", tmp, treeSum)
        break
    preMiddle = tmp
    #print(middle, end=" ")
    if(middle == preMiddle):
        for i in range(m):
            ...
            treeSum = 0
            for k in range(n):
                cutTree = trees[k] - middle;
                if(cutTree > 0):
                    treeSum += cutTree
            #print("calcing... ", middle, treeSum)
            if treeSum < m:
                middle = middle - 1
                break
        break
