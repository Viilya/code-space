'''
문제
지훈이는 최근에 혼자 하는 카드게임을 즐겨하고 있다. 게임에 사용하는 각 카드에는 양의 정수 하나가 적혀있고 같은 숫자가 적힌 카드는 여러 장 있을 수 있다. 

게임방법은 우선 짝수개의 카드를 무작위로 섞은 뒤 같은 개수의 두 더미로 나누어 하나는 왼쪽에 다른 하나는 오른쪽에 둔다. 그리고 빈 통을 하나 준비한다. 

이제 각 더미의 제일 위에 있는 카드끼리 서로 비교하며 게임을 한다. 게임 규칙은 다음과 같다. 지금부터 왼쪽 더미의 제일 위 카드를 왼쪽 카드로, 
...
n = int(input())
listL = list(map(int, read().split()))
listR = list(map(int, read().split()))
listchecked = [[-1 for i in range(n+1)] for s in range(n+1)]



def dp(leftcnt, rightcnt):
    ## end conditions, already visited or counted all
    if leftcnt > n - 1 or rightcnt > n - 1:
        ##4print("CP 1 / ", leftcnt, rightcnt, " / ", listL[leftcnt], listR[rightcnt])
        return 0
    if listchecked[leftcnt][rightcnt] != -1:
        ##print("CP 2 / ", leftcnt, rightcnt, " / ", listL[leftcnt], listR[rightcnt])
        return listchecked[leftcnt][rightcnt]

    ## L > R, throw R to bin
    if listL[leftcnt] > listR[rightcnt] :
        ##print("CP 3 / ", leftcnt, rightcnt, " / ", listL[leftcnt], listR[rightcnt])
        listchecked[leftcnt][rightcnt] = listR[rightcnt] + dp(leftcnt, rightcnt+1)

    ## R < L, throw L + R or only L to bin
    else : 
        ##print("CP 4 / ", leftcnt, rightcnt, " / ", listL[leftcnt], listR[rightcnt])
        listchecked[leftcnt][rightcnt] = max(
            dp(leftcnt+1, rightcnt), dp(leftcnt+1, rightcnt+1)
        )
    return listchecked[leftcnt][rightcnt]
