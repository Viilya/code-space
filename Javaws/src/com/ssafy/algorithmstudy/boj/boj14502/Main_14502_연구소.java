...
public class Main_14502_연구소 {
            ...
            if(k != currX) {  // x 좌표가 같지 않은 경우 처음부터 쭉 돌기
                for (int s = 0; s < M; s++) {
                    if (map[k][s] == 0) {
                        ...
                        mapCopy(map, nextMap);
                        nextMap[k][s] = 1;
                        buildWallAndBFS(nextMap, count + 1, k, s);
                    }
                }
            }
        ...
        for(int k = 0 ; k < viruses.size(); k ++){ // 모든 바이러스 위치를 탐색
            ...
            while(!q.isEmpty()){ // 바이러스 BFS
                ...
                for(int s = 0 ; s < 4; s ++){ // 4방 탐색하며 BFS
                    ...
                    if(isSafe(X, Y, map)){
                        q.add(new Point(X, Y));
                        map[X][Y] = 2; // 빈 공간인 경우 2로 채우고 count 줄이기
                        count--;
                    }
                }
            }
    ...
    public static void mapCopy(int mapSource[][], int mapDestination[][]){ // map 복사
        for(int k = 0 ; k < N ; k++){
            mapDestination[k] = Arrays.copyOf(mapSource[k], M);
        }
    }
